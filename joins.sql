...
-- Resultados para las consultas propuestas en el ejercicio de Joins. El detalle del ejercicio propuesto se encuentra en el archivo README
...
select clients.id, clients.first_name, clients.last_name, clients.email, accounts.account_no, accounts.balance from clients left join accounts on clients.id = client_id ;
select accounts.account_no, accounts.client_id, account_types.name from accounts left join account_types on accounts.type = account_types.id;
select transactions.id, transactions.amount, account_types.name from transactions right join account_types on transactions.transaction_type = account_types.id ;
select transactions_types.id, transactions.amount, transactions_types.name from transactions_types right join transactions on transactions_types.id = transactions.transaction_type;
...
create view v_transactions_detail as select transactions.id, transactions.amount, account_types.name from transactions right join account_types on transactions.transaction_type = account_types.id ;
create view v_transations_types_detail as select transactions_types.id, transactions.amount, transactions_types.name from transactions_types right join transactions on transactions_types.id = transactions.transaction_type;
